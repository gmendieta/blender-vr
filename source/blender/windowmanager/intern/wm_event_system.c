      ...
      if (note->window == win) {
        if (note->category == NC_SCREEN) {
          if (note->data == ND_WORKSPACE_SET) {
            ...
            // Lets try to reset the VR region when workspace is changed
            vr_region_set(NULL);
    ...
    while ((event = win->queue.first)) {
      ...
      if ((action & WM_HANDLER_BREAK) == 0) {
        ...
        ED_screen_areas_iter(win, screen, sa)
        {
          ...
          if (wm_event_inside_rect(event, &sa->totrct) || vr_event_inside_win) {
            CTX_wm_area_set(C, sa);

            if ((action & WM_HANDLER_BREAK) == 0) {
              for (ar = sa->regionbase.first; ar; ar = ar->next) {
                int vr_event_inside_region = ar == vr_region_get() && event->vr_data != NULL;
                if (wm_event_inside_region(event, ar) || vr_event_inside_region) {
                  CTX_wm_region_set(C, ar);
...
static void update_vr_data(wmWindow *win, wmEvent *event)
{
	const GHOST_VRData *vrd = GHOST_getVRData(win->ghostwin);
